...
BASETEN_API_KEY=<bot-only Baseten key>
...
literal `Auto` reason is also treated as omitted while AI is enabled. A Baseten DeepSeek planning
...
Only compact titles, URLs, and excerpts enter the next Baseten call. That synthesis call may request
...
GIFs, videos, avatars, banners, server art, or media URLs to Baseten or Brave. It removes profile/server
...
AI context. Planning uses low reasoning with a 4,096-token completion budget. Synthesis uses medium
...
reporter to supply the final maximum-512-character text, makes no Baseten or Brave call, and omits
...
encrypted, expiring bot draft. Baseten token/request usage and Brave request counts are accumulated per
